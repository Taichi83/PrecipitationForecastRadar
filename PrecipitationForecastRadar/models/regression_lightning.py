...
class Precip_regression_base(UNet_base):
    ...
    def __init__(self, dataset_dir: str, num_input_images: int = 12, num_output_images: int = 6,
                 valid_size: float = 0.1, batch_size: int = 6,
        ...
        self.use_oversampled_dataset = use_oversampled_dataset
    ...
    def prepare_data(self):
        ...
        if self.use_oversampled_dataset:
            self.train_dataset = dataset_precip.precipitation_maps_oversampled_h5(
                in_file=self.dataset_dir, num_input_images=self.num_input_images,
                num_output_images=self.num_output_images, train=True,
    ...
    def val_dataloader(self):
        valid_loader = torch.utils.data.DataLoader(
            self.valid_dataset, batch_size=self.batch_size, sampler=self.valid_sampler,
            num_workers=1, pin_memory=True
        )
        return valid_loader
